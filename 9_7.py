...
mario_back = pygame.image.load('mario_back_new2.png')
...
class Mario(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.do_fall == True:
            self.fall()
            return

        if self.is_jump == True:   # and self.is_on == False:
            self.jump_update()
...
class Turtle(pygame.sprite.Sprite):
    def __init__(self,x,y,img,loop_count):
        ...
        self.image = img
        self.image.set_colorkey((255,255,255))
        self.image = self.img.subsurface(turtle_rect_walk1_left)
        self.rect = self.image.get_rect()
        self.rect.left = x
        self.rect.y = y
        self.loop_count = loop_count
        self.turtle_face_turn = 0
        self.walk_step = 0
        self.is_alive = True
    ...
    def update(self):
        ...
        turtle_rect_walk_left = [pygame.Rect(0,0,20,27) , pygame.Rect(50,0,20,27)]
        turtle_rect_walk_right = [pygame.Rect(100,0,20,27) , pygame.Rect(150,0,20,27)]
        turtle_die_rect = pygame.Rect(77,30,20,20)

        if self.is_alive == False:
            self.image = self.img.subsurface(turtle_die_rect)
            return

        if self.turtle_face_turn == 0:
            self.image = self.img.subsurface(turtle_rect_walk_left[self.walk_step])
            self.walk_step += 1
            self.walk_step %= 2
        elif self.turtle_face_turn == 1:
            self.image = self.img.subsurface(turtle_rect_walk_right[self.walk_step])
            self.walk_step += 1
            self.walk_step %= 2
        #self.is_walk = False
...
turtle_img = pygame.image.load(os.path.join('enemies.gif')).convert()
turtle = [Turtle(700,525,turtle_img,10),Turtle(800,525,turtle_img,5),Turtle(1200,525,turtle_img,7),Turtle(1500,525,turtle_img,12)]
...
blocks = [ Block(524, 480, 62, 66) ]   #482   480
...
def check_collide():
    ...
    for e in enemy:
        if mario.is_fall == True:
            e.dead()
        elif e.is_alive == True:
            mario.dead()


    blocks = pygame.sprite.spritecollide(mario,block_group,False)

    if len(blocks) == 0:
        if mario.is_on == True:   # and mario.do_fall == False:
            mario.do_fall = True
        #else:
        #    mario.do_fall = False
        #    mario.is_on = False
    ...
    for b in blocks:
        if mario.is_fall == True:
            ...
            if mario.is_on == False:
                if mario.mario_face_turn == 0:
                    if screen_move == False:
                        mario.move(10)
                    else:
                        mario.move(20)
                else:
...
while not done:   #game loop  (ให้ใช้เวลาน้อยสุด)
    ...
    if mario.get_x() > 400 or mario.get_x() < 200: #background.get_x() + 400: #and hit_block == False:
